...
set_initial_condition <- function(o, vals){
  if(o$connected == F || is.null(o))
    stop("Error, no connection made. Need to call pysd_connect() before set_components()")
  if(o$loaded_model == F)
    stop("Error, no model loaded...")
  UseMethod("set_initial_condition")
}

set_initial_condition.ipysd <- function(o, vals){
  conv <- reticulate::r_to_py(vals)
  o$model$set_initial_condition(initial_condition = tuple(0, conv))
}

###

run_model_v2 <- function(o, vals){
  if(o$connected == F || is.null(o))
    stop("Error, no connection made. Need to call pysd_connect() befoe run_model()")
  if(o$loaded_model == F)
    stop("Error, no model loaded...")
  UseMethod("run_model_v2")
}

#' @export
run_model_v2.ipysd <- function(o, vals){
  conv <- reticulate::r_to_py(vals)
  out <- tibble::as_data_frame(o$model$run(initial_condition = tuple(0, conv)))
}

###

run_model_v3 <- function(o, vals){
  if(o$connected == F || is.null(o))
    stop("Error, no connection made. Need to call pysd_connect() befoe run_model()")
  if(o$loaded_model == F)
    stop("Error, no model loaded...")
  UseMethod("run_model_v3")
}

#' @export
run_model_v3.ipysd <- function(o, vals){
  conv <- reticulate::r_to_py(vals)
  out <- tibble::as_data_frame(o$model$run(params = conv))
}

###

run_model_v4 <- function(o, vals){
  if(o$connected == F || is.null(o))
    stop("Error, no connection made. Need to call pysd_connect() befoe run_model()")
  if(o$loaded_model == F)
    stop("Error, no model loaded...")
  UseMethod("run_model_v4")
}

#' @export
run_model_v4.ipysd <- function(o, ini, vals){
  conv_ini <- reticulate::r_to_py(ini)
  conv_vals <- reticulate::r_to_py(vals)
  out <- tibble::as_data_frame(o$model$run(params = conv_vals, initial_condition = tuple(0, conv_ini)))
}
